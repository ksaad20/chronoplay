"""Media asset validation and metadata primitives for ChronoPlay."""
...
class MediaAsset:
    """Describe a media asset used by ChronoPlay.

    The class deliberately contains no decoder or playback logic. It provides
    the stable media boundary consumed by the scheduling and future playout
    layers.
    """
    ...
    def identifier(self) -> str:
        """Return the explicit media ID or a path-derived identifier."""
    ...
    def extension(self) -> str:
        """Return the normalized media-file extension."""
    ...
    def validate_path(self) -> None:
        """Validate that the referenced asset exists and is readable."""
        ...
        try:
            ...
            raise MediaUnreadableError(f"Media asset is not readable: {self.path}") from exc
    ...
    def validate(self, *, require_supported_extension: bool = True) -> None:
        ...
        if require_supported_extension and not self.is_supported:
            raise MediaValidationError(f"Unsupported media format: {self.extension or '<none>'}")


@dataclass(frozen=True, slots=True)
class MediaLibrary:
    """Represent the root directory containing ChronoPlay media assets."""

    root: Path

    def __post_init__(self) -> None:
        """Normalize the media-library root."""
        root = Path(self.root)
    ...
    def resolve(self, path: str | Path) -> Path:
        """Resolve a media path relative to the library root."""
    ...
    def asset(
        ...
        *,
        ...
        """Create a MediaAsset resolved against the library root."""
        return MediaAsset(
    ...
    def validate(self, path: str | Path) -> MediaAsset:
        """Resolve and validate a media asset."""
