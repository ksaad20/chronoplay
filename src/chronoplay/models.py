...
class MediaItem:
    ...
    def __post_init__(self) -> None:
        ...
        if not normalized_path.as_posix().strip():
...
class ScheduleEntry:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(
            self,
            "start",
            _normalize_datetime(self.start, "start"),
        )
...
class Channel:
    ...
    def __post_init__(self) -> None:
        ...
        if not all(isinstance(entry, ScheduleEntry) for entry in self.entries):
            raise ModelValidationError("Channel entries must contain only ScheduleEntry objects.")
...
class PlayoutCommand:
    ...
    def __post_init__(self) -> None:
        ...
        if self.action == PlayoutAction.PLAY and self.media is None:
            raise ModelValidationError("PLAY commands require a media item.")
