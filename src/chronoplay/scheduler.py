...
The scheduler is intentionally independent of media playback. It provides the
time and ordering foundation on which later playout components can operate.
"""
...
class ScheduleEvent:
    ...
    ----------
    start:
        Absolute, timezone-aware execution time. Internally this is normalized
        to UTC.
    duration:
...
class Scheduler:
    ...
    sequence. Consequently, two events with identical timestamps execute in
    the same order in which they were scheduled.

    The scheduler does not itself perform media playback. Callbacks receive
    the :class:`ScheduleEvent` and are responsible for executing its payload.
    """
    ...
    def cancel(self, event_id: str) -> ScheduleEvent:
        ...
        Cancellation is lazy: the heap entry is retained until it reaches the
        head of the queue, avoiding an O(n) heap mutation.
        """
        with self._lock:
    ...
    def pop_next(self) -> ScheduleEvent | None:
        ...
        Returns ``None`` when no pending events remain.
        """
        with self._lock:
            while self._queue:
    ...
    def run(
        ...
        *,
        ...
        sleep_interval:
            Maximum polling interval in seconds when waiting for the next
            event.
        """
        if sleep_interval <= 0:
