...
def _parse_duration(value: Any, field_name: str) -> timedelta | None:
    ...
    if not isinstance(value, str):
        raise ScheduleParseError(
            f"{field_name} must be seconds or HH:MM:SS."
        )
    ...
    if len(parts) != 3:
        raise ScheduleParseError(
            f"{field_name} must use HH:MM:SS format."
        )
    ...
    if minutes >= 60 or seconds >= 60:
        raise ScheduleParseError(
            f"{field_name} contains invalid minutes or seconds."
        )
...
def _parse_media(
    ...
    if not isinstance(metadata, dict):
        raise ScheduleParseError(
            f"schedule entry {entry_index} media.metadata "
            "must be a mapping."
        )
    ...
    try:
        ...
        raise ScheduleParseError(
            f"Invalid media in schedule entry {entry_index}: {exc}"
        ) from exc
...
def _parse_entry(
    ...
    if not isinstance(enabled, bool):
        raise ScheduleParseError(
            f"schedule entry {entry_index} enabled must be boolean."
        )
    ...
    if not isinstance(metadata, dict):
        raise ScheduleParseError(
            f"schedule entry {entry_index} metadata must be a mapping."
        )
    ...
    try:
        ...
        raise ScheduleParseError(
            f"Invalid schedule entry {entry_index}: {exc}"
        ) from exc
...
def load_schedule(
    ...
    try:
        ...
        raise ScheduleFileError(
            f"Unable to read schedule file: {schedule_path}"
        ) from exc
    except yaml.YAMLError as exc:
        raise ScheduleParseError(
            f"Unable to parse YAML schedule: {schedule_path}"
        ) from exc
    ...
    if schedule_data is None:
        raise ScheduleParseError(
            "schedule document must contain a 'schedule' field."
        )
    ...
    for index, raw_entry in enumerate(schedule_data, start=1):
        ...
        if entry.entry_id in entry_ids:
            raise ScheduleParseError(
                f"Duplicate schedule entry ID: {entry.entry_id}"
            )
