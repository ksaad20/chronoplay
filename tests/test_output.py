...
def test_output_exposes_media_output_interface() -> None:
"""FFmpegOutput should expose the MediaOutput interface."""
output = FFmpegOutput(make_config())

```
assert callable(output.start)
assert callable(output.stop)
assert callable(output.health)
assert isinstance(output.state, OutputState)
```
...
TimeoutExpired(cmd="ffmpeg", timeout=5),
None,
]
mock_popen.return_value = process
